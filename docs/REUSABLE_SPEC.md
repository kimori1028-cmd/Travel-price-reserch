...
- 推移グラフ・最安/最高値（＋その日付）・値動きの明細（いつ・いくらから・いくらに）・カレンダーの値上がり/値下がり矢印は、すべてこの履歴からフロントで復元する（`trend.ts` の `buildTrend`/`trendStats`/`trendChanges`/`recentTotalChange`）。
...
- **推移グラフ**（`TrendChart` / `trend.ts`）: 履歴の変化イベントからN泊合計のステップ折れ線を復元。最安（緑）・最高（赤）の点と**その日付**を表示。さらに**値動き明細**（`trendChanges`）を新しい順のリストで表示し、「**何日に・いくらから・いくらへ**（↓下落=緑／↑上昇=赤・差額付き）」を一目で追えるようにする。詳細シートは全グレードのカードを並べ、各カードの推移は**そのグレードの履歴のみ**（`history.filter(room_grade===g.key)`）から復元する。
