...
- **推移グラフ**（`TrendChart` / `trend.ts`）: 履歴の変化イベントからN泊合計のステップ折れ線を復元。最安（緑）・最高（赤）の点と**その日付**を表示。さらに**値動き明細**（`trendDailyChanges`）を新しい順のリストで表示し、「**何日に・いくらから・いくらへ**（↓下落=緑／↑上昇=赤・差額付き）」を一目で追えるようにする。1時間ごとのバッチで同じ日に複数回動くことがあるため、**同じ日は1行に集約**し「その日の始値→終値・1日の正味差額」を表示する（日付は必ず1回だけ／正味ゼロの日は除外）。詳細シートは全グレードのカードを並べ、各カードの推移は**そのグレードの履歴のみ**（`history.filter(room_grade===g.key)`）から復元する。
