...
- **推移グラフ**（`TrendChart` / `trend.ts`）: 履歴の変化イベントからN泊合計のステップ折れ線を復元。最安（緑）・最高（赤）の点と**その日付**を表示。さらに**値動き明細**（`trendChanges`）を新しい順のリストで表示し、「**何日に・いくらから・いくらへ**（↓下落=緑／↑上昇=赤・差額付き）」を一目で追えるようにする。詳細シートはカレンダーで**選択中のグレードだけ**を表示する（ヴィラ選択時はヴィラのみ）。
