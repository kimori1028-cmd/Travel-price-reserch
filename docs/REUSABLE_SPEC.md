...
- 推移グラフ・最安/最高値はこの履歴からフロントで復元（ステップ折れ線）。
...
- **カレンダー**: 月表示。泊数プルダウン、グレード複数選択（V/S/P 頭文字＋`¥金額`、月内最安を緑）、左右スワイプで月移動、セルタップで詳細ボトムシート（×ボタン＋各泊内訳＋推移グラフ＋予約リンク＋★保存）。
- **最安検索**: 泊数/曜日/グレード（複数選択）＋**月単位の期間プルダウン（日本語）**。DB上で計算しAPIは叩かない。安い順表示。
- **お気に入り**: 保存/共有トグル/通知閾値設定/削除。保存時価格との差分・これまで最安を表示。
- **認証**: 招待・再設定リンクからの**自前パスワード設定画面**、パスワード表示切替、「パスワードを忘れた」再送。
- **予約リンク**: 「サイトで確認」（宿泊のみ・日付入り）＋「ANA楽パック」（便別ボタン）。すべて**楽天公式ドメインへ直リンク**（アフィリエイト/トラッキングID無し）。
- **PWA**: `manifest.webmanifest`＋アイコンでホーム画面に追加可能（standalone）。外部リンクが**アプリ画面を置換しない**ため、スマホで「アプリが閉じる」現象を回避。
- **デモモード**: Supabase 未接続時はバンドルしたサンプルJSONで動作（見た目確認用）。
...
- [ ] **航空便番号**（`nsBinOuro/nsBinHukuro`。季節ダイヤで変わる。不要なら楽パックボタン自体を削除）
