...
3. 以後は自動で動きます:
   - **毎時15分**: お気に入りに登録された日程だけを更新（数分で完了）
   - **毎晩 3:00 JST**: 1年分をフル更新
   - チェックイン日を過ぎたお気に入り・価格・履歴は自動削除されます
...
アプリのお気に入りで「📧 通知」をタップして金額を入力すると、その日程の参考価格が
**設定した金額以下になった時**にメールが届きます（同じ価格で繰り返し通知はされません）。
メールを送るには送信元のGmail設定が必要です（1回だけ）:

1. **Gmailのアプリパスワードを発行**:
   - Googleアカウントで2段階認証を有効にする（Google アカウント → セキュリティ）
   - https://myaccount.google.com/apppasswords を開き、アプリパスワードを作成（16文字の英字が発行される）
2. **GitHub Secrets に追加**（Settings → Secrets and variables → Actions）:

   | Name | 値 |
   |---|---|
   | `SMTP_USERNAME` | あなたのGmailアドレス |
   | `SMTP_PASSWORD` | 発行したアプリパスワード（16文字、スペース無し） |
   | `APP_URL` | （任意）VercelのアプリURL。メール本文にリンクが入る |

- 通知先は各ユーザーのログイン用メールアドレスです（Supabaseに登録したもの）。
- この設定をしなくてもアプリは正常に動きます（通知だけスキップされます）。
...
- **プランの絞り込み**: 同ファイルの `squeeze_condition` で指定します（現在は `breakfast` = 朝食付きプランのみ）。素泊まりも含めた最安にしたい場合はこの行を削除してください。
